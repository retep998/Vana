...
if isQuestActive(2056) then
	if getItemAmount(4031040) > 0 then
		addText("Looking carefully into Treasure Chest, there seems to be nothing there.");
		sendNext();

		setMap(103000100);
	else
		addText("Looking carefully into Treasure Chest, there seems to be a stack of papers in there");

		if hasOpenSlotsFor(4031040) then
			addText(". Reached out my hand and viola, a huge stack of money.");
			giveItem(4031040, 1);
			sendNext();
