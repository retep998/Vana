...
elseif state == 1 then
	...
	if what == 0 then
		sendStyle(skins, 5);
	elseif what == 1 then
		haircolour = getHair() % 10;
		newhairs = {};
		for i = 1, #hairs do
			if not (hairs[i] + haircolour == getHair()) then
	...
	elseif what == 3 then
		eyecolour = (getEyes() % 1000) - (getEyes() % 100);
		neweyes = {};
		for i = 1, #eyes do
			if not (eyes[i] + eyecolour == getEyes()) then
				neweyes[#neweyes+1] = eyes[i] + eyecolour;
			end
		end
