...
if (getItemAmount(4031009) > 0 and getItemAmount(4031013) == 0) then	
	addText("Hmmm...it is definitely the letter from #b#p1032001##k...so you came all the way here to take ");
	addText("the test and make the 2nd job advancement as the magician. Alright, I'll explain the test to you. ");
	...
	if yes == 0 then
		addText("I don't think you are prepared for this. Talk to me when you make yourself much more ready for this. ");
		addText("There are neither portals nor stores inside, so you better get ready for it all the way.");
		sendNext();
	else
		addText("Alright, I'll let you in! Once you go in, defeat the monsters and collect 30 Dark Marbles. After ");
		addText("that go talk to my colleague inside to receive #b#t4031012##k as proof that you have passed ")
		addText("the test. Best of luck to you.");
		sendNext();
		setMap(108000201);
	end
...
elseif (getJob() == 200 and getLevel() >= 30) then
	addText("Do you want to be a stronger magician? Let me take care of that for you, then. You look definitely ");
	addText("qualified for it. For now, go see #b#p1032001##k of Ellinia first.");
	sendNext();
end