...
if getItemAmount(4031011) > 0 and getItemAmount(4031013) == 0 then
	addText("Hmmm...it is definitely the letter from " .. blue(npcRef(1052001)) .. "...so you came all the way here to take the test and make the 2nd job advancement as the rogue. ");
	...
	if answer == answer_no then
		...
		addText("Defeat the monster inside to earn 30 Dark Marble and then talk to my colleague inside; he'll give you " .. blue(itemRef(4031012)) .. " as a proof that you've passed the test.");
...
elseif getJob() == 400 and getLevel() >= 30 then
	addText("Do you want to be a stronger thief? ");
	addText("Let me take care of that for you, then. ");
	addText("You look definitely qualified for it. ");
	addText("For now, go see " .. blue(npcRef(1052001)) .. " of Kerning City first.");
	sendNext();
else
	-- TODO FIXME text?
end