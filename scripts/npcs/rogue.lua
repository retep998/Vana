...
elseif ((getJob() == 400 and getLevel() < 30) or getJob() == 410 or getJob() == 420 or getJob() == 411 or getJob() == 421 or getJob() == 412 or getJob() == 422) then
	if isQuestActive(6141) then
		addText("Would you like to attempt the quest?");
		yes = askYesNo();
		if yes == 1 then
			if not isInstance("ninjaAmbush") then
				createInstance("ninjaAmbush", 20 * 60, true);
				addInstancePlayer(getID());
				setMap(910300000);
			else
				addText("Someone else is attempting the quest.");
				sendNext();
			end
		else
			addText("I see. Come to me when you're prepared.");
			sendNext();
		end
	else
