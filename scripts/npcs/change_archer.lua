...
if getItemAmount(4031010) > 0 and getItemAmount(4031013) == 0 then
	addText("Hmmm...it is definitely the letter from " .. blue(npcRef(1012100)) .. "...so you came all the way here to take the test and make the 2nd job advancement as the bowman. ");
	...
	if answer == answer_no then
		...
		addText("There are neither portals nor stores inside, so you better get 100% ready for it.");
		sendNext();
	else
		addText("Alright I'll let you in! ");
		addText("Defeat the monsters inside, collect 30 Dark Marbles, and then talk to my colleague inside. ");
		addText("Then he'll award you the proof of passing the test, " .. blue(itemRef(4031012)) .. ". ");
		addText("Good luck.");
		sendNext();
...
elseif getJob() == 300 and getLevel() >= 30 then
	addText("Do you want to be a stronger bowman? ");
	addText("Let me take care of that for you, then. ");
	addText("You look definitely qualified for it. ");
	addText("For now, go see " .. blue(npcRef(1012100)) .. " of Henesys first.");
	sendNext();
else
	-- TODO FIXME text?
end