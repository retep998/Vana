...
if (getItemAmount(4031010) > 0 and getItemAmount(4031013) == 0) then	
	addText("Hmmm...it is definitely the letter from #b#p1012100##k...so you came all the way here to take ");
	addText("the test and make the 2nd job advancement as the bowman. Alright, I'll explain the test to you. ");
	addText("Don't sweat it much, though; it's not that complicated.");
	sendNext();

	addText("I'll send you to a hidden map. You'll see monsters not normally seen in normal fields. They look the same like ");
	...
	if yes == 0 then
		addText("I don't think you are prepared for this. Find me when you ARE ready. There are neither portals ");
		addText("nor stores inside, so you better get 100% ready for it.");
		sendNext();
	else
		addText("Alright I'll let you in! Defeat the monsters inside, collect 30 Dark Marbles, and then talk to my ");
		addText("colleague inside. Then he'll award you the proof of passing the test, #b#t4031012##k. ");
		addText("Good luck.");
		sendNext();
		setMap(108000102);
	end
elseif (getItemAmount(4031010) > 0 and getItemAmount(4031013) > 0) then
	...
	if yes == 0 then
		...
		addText("conversation with a colleague of mine inside. He'll give you the #b#t4031012##k, the ");
...
elseif (getJob() == 300 and getLevel() >= 30) then
	addText("Do you want to be a stronger bowman? Let me take care of that for you, then. You look definitely ");
	addText("qualified for it. For now, go see #b#p1012100##k of Henesys first.");
	sendNext();
end