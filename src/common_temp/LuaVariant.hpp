...
#include "common_temp/hash_combine.hpp"
#include "common_temp/LuaType.hpp"
...
namespace vana {
	...
	public:
		...
		auto get_type() const -> lua::lua_type;
		auto is(lua::lua_type type) const -> bool;
		auto is_any_of(init_list<lua::lua_type> types) const -> bool;
	...
	private:
		...
		struct hash {
			auto operator()(const lua_variant &v) const -> size_t {
				switch (v.get_type()) {
					case lua::lua_type::nil: return utilities::misc::hash_combinator(0);
					case lua::lua_type::boolean: return utilities::misc::hash_combinator(v.as<bool>() ? 0x55555555 : 1);
					case lua::lua_type::number: return utilities::misc::hash_combinator(v.as<double>());
					case lua::lua_type::string: return utilities::misc::hash_combinator(v.as<string>());
		...
		struct equal_to {
			auto operator()(const lua_variant &l, const lua_variant &r) const -> bool {
				...
				switch (l.get_type()) {
					case lua::lua_type::nil: return true;
					case lua::lua_type::boolean: return l.as<bool>() == r.as<bool>();
					case lua::lua_type::number: return l.as<double>() == r.as<double>();
					case lua::lua_type::string: return l.as<string>() == r.as<string>();
					default: throw not_implemented_exception{"lua_type"};
				}
			}
		};
...
namespace std {
	template <>
	struct hash<vana::lua_variant> {
		auto operator()(const vana::lua_variant &v) const -> size_t {
			return vana::lua_variant::hash{}(v);
		}
	};

	template <>
	struct equal_to<vana::lua_variant> {
		auto operator()(const vana::lua_variant &l, const vana::lua_variant &r) const -> bool {
			return vana::lua_variant::equal_to{}(l, r);
		}
	};

	template <>
	struct less<vana::lua_variant> {
		auto operator()(const vana::lua_variant &l, const vana::lua_variant &r) const -> bool {
			return l < r;
		}
	};
	...
	template <>
	struct greater<vana::lua_variant> {
		auto operator()(const vana::lua_variant &l, const vana::lua_variant &r) const -> bool {
			return l > r;
		}
	};
...
namespace vana {
	...
	auto operator ==(const lua_variant &a, const lua_variant &b) -> bool {
		...
		switch (a.m_type) {
			case lua::lua_type::nil: return true;
			case lua::lua_type::boolean: return a.as<bool>() == b.as<bool>();
			case lua::lua_type::number: return a.as<double>() == b.as<double>();
			case lua::lua_type::string: return a.as<string>() == b.as<string>();
			default: throw not_implemented_exception{"lua_type"};
		}
	}
	...
	auto operator >(const lua_variant &a, const lua_variant &b) -> bool {
		if (a.m_type != b.m_type) return false;
		switch (a.m_type) {
			case lua::lua_type::nil: return false;
			case lua::lua_type::boolean: return false;
			case lua::lua_type::number: return a.as<double>() > b.as<double>();
			case lua::lua_type::string: return a.as<string>() > b.as<string>();
			default: throw not_implemented_exception{"lua_type"};
		}
	...
	auto lua_variant::as_impl<bool>(bool *) const -> bool {
		if (m_type != lua::lua_type::boolean) throw std::invalid_argument{"Value is not a boolean"};
		return m_boolean;
	}

	template <>
	auto lua_variant::as_impl<int8_t>(int8_t *) const -> int8_t {
		if (m_type != lua::lua_type::number) throw std::invalid_argument{"Value is not an integer"};
	...
	auto lua_variant::as_impl<double>(double *) const -> double {
		if (m_type != lua::lua_type::number) throw std::invalid_argument{"Value is not a double"};
		return m_number;
	}

	template <>
	auto lua_variant::as_impl<string>(string *) const -> string {
		if (m_type != lua::lua_type::string) throw std::invalid_argument{"Value is not a string"};
		return m_string;
	}
	...
	auto lua_variant::as_impl(vector<TElement> *) const -> vector<TElement> {
		if (m_type != lua::lua_type::table) throw std::invalid_argument{"Value is not an array"};
	...
	auto lua_variant::as_impl(hash_map<TKey, TElement, THash, TOperation> *) const -> hash_map<TKey, TElement, THash, TOperation> {
		if (m_type != lua::lua_type::table) throw std::invalid_argument{"Value is not a key-value pair store"};
