...
// Miscellaneous
auto lua_exports::get_distance_npc(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	env.push<int32_t>(lua_vm, get_player(lua_vm, env)->get_pos() - get_npc(lua_vm, env)->get_pos());
	return 1;
}

auto lua_exports::get_npc_id(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	env.push<game_npc_id>(lua_vm, get_npc(lua_vm, env)->get_npc_id());
	return 1;
}

auto lua_exports::run_npc_npc(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
...
auto lua_exports::show_storage(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
...
// NPC interaction
auto lua_exports::add_text(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	get_npc(lua_vm, env)->add_text(env.get<string>(lua_vm, 1));
	return 0;
}

auto lua_exports::ask_accept_decline(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	get_npc(lua_vm, env)->send_accept_decline();
	return env.yield(1);
}

auto lua_exports::ask_accept_decline_no_exit(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	get_npc(lua_vm, env)->send_accept_decline_no_exit();
	return env.yield(1);
}

auto lua_exports::ask_choice(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	get_npc(lua_vm, env)->send_simple();
	return env.yield(1);
}

auto lua_exports::ask_number(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
...
auto lua_exports::ask_question(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
...
auto lua_exports::ask_quiz(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
...
auto lua_exports::ask_style(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
...
auto lua_exports::ask_text(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	int16_t min = 0;
	int16_t max = 0;
	string def = "";
	if (env.is(lua_vm, 1, lua_type::number) && env.is(lua_vm, 2, lua_type::number)) {
		min = env.get<int32_t>(lua_vm, 1);
		max = env.get<int32_t>(lua_vm, 2);
		if (env.is(lua_vm, 3, lua_type::string)) {
			def = env.get<string>(lua_vm, 3);
		}
	}
	else if (env.is(lua_vm, 1, lua_type::string)) {
...
auto lua_exports::ask_yes_no(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	get_npc(lua_vm, env)->send_yes_no();
	return env.yield(1);
}

auto lua_exports::send_back_next(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	get_npc(lua_vm, env)->send_dialog(true, true);
	return env.yield(0);
}

auto lua_exports::send_back_ok(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	get_npc(lua_vm, env)->send_dialog(true, false);
	return env.yield(0);
}

auto lua_exports::send_next(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	get_npc(lua_vm, env)->send_dialog(false, true);
	return env.yield(0);
}

auto lua_exports::send_ok(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
...
// Quest
auto lua_exports::add_quest(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
...
auto lua_exports::end_quest(lua_State *lua_vm) -> lua_return {
	auto &env = get_environment(lua_vm);
	game_quest_id quest_id = env.get<game_quest_id>(lua_vm, 1);
