...
namespace InventoryPacket {
	...
	void addItem(Player *player, int8_t inv, int16_t slot, Item *item, bool fromDrop);
	void addNewItem(Player *player, int8_t inv, int16_t slot, Item *item, bool fromDrop);
	...
	void sendItemExpired(Player *player, const vector<int32_t> &items);
}