...
bool PlayerQuests::giveRewards(int16_t questid, bool start) {
	...
	if (questinfo.rewards.jobrewards.find(m_player->getStats()->getJob()) != questinfo.rewards.jobrewards.end()) {
		for (i = 0; i < questinfo.rewards.jobrewards[m_player->getStats()->getJob()].size(); i++) {
			info = questinfo.rewards.jobrewards[m_player->getStats()->getJob()][i];
			if ((info.start && start) || (!info.start && !start)) {
				if (info.isitem && info.prop > 0) {
					chance += info.prop;
					items.push_back(info);		
