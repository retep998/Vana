...
uint8_t PlayerBuddyList::add(const string &name) {
	...
	mysqlpp::Query query = Database::getCharDB().query();
	query << "SELECT c.id, u.gm, c.buddylist_size AS buddylist_limit,(SELECT COUNT(b.id) FROM buddylist b WHERE b.charid = c.id ) AS buddylist_size FROM characters c INNER JOIN users u ON c.userid = u.id WHERE c.name = " << mysqlpp::quote << name;
	mysqlpp::StoreQueryResult res = query.store();
	...
	if ((int32_t) res[0][1] > 0 && !player->isGM()) { // GM cannot be in buddy list unless the player is a GM
		return 0x0e;
	}

	if ((uint32_t) res[0][3] >= (uint32_t) res[0][2]) { // Opposite-end buddy list full
		return 0x0c;
	}
	...
	query << "SELECT buddylist.buddy_charid as buddy_charid, characters.name as name FROM buddylist INNER JOIN characters ON buddylist.buddy_charid = characters.id WHERE buddylist.id = " << res2.insert_id();
	res = query.store();
