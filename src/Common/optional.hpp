...
namespace vana {
	...
	template <typename TElement>
	struct packet_serialize<utilities::misc::optional<TElement>> {
		auto read(packet_reader &reader) -> utilities::misc::optional<TElement> {
			utilities::misc::optional<TElement> ret;
			if (reader.get<bool>()) {
				ret = reader.get<TElement>();
			}
			return ret;
		};
		auto write(packet_builder &builder, const utilities::misc::optional<TElement> &obj) -> void {
	...
	template <typename TElement>
	using optional = utilities::misc::optional<TElement>;
