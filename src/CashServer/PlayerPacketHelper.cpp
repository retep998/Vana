...
void PlayerPacketHelper::addCashItemInfo(PacketCreator &packet, CashItem *item) {
	// 55
	packet.add<int64_t>(item->getId());
	packet.add<int32_t>(item->getUserId());
	packet.add<int32_t>(0);
	packet.add<int32_t>(item->getItemId());
	packet.add<int32_t>(0); 
...
void PlayerPacketHelper::addModifiedCashInfo(PacketCreator &packet, ModifiedCashItem item) {
	int32_t flags = item.flags;
	packet.add<int32_t>(item.serial);
	packet.add<int32_t>(flags);
	if (flags & 0x1) {
		packet.add<int32_t>(0);
	}
	if (flags & 0x2) {
		packet.add<int16_t>(12);
	}
	if (flags & 0x10) {
		packet.add<int8_t>(0x0F);
	}
	if (flags & 0x4) {
		packet.add<int32_t>(item.discountPrice);
	}
	if (flags & 0x8) {
		packet.add<int8_t>(1);
	}
	...
	if (flags & 0x400) {
		packet.addBool(item.showUp);
	}
	if (flags & 0x800) {
		packet.add<int8_t>(item.mark);
	}
	if (flags & 0x1000) {
		packet.add<int8_t>(1);
	}
	...
	if (flags & 0x10000) {
		packet.add<int8_t>(item.items.size());
		for (size_t i = 0; i < item.items.size(); i++) {
			packet.add<int32_t>(item.items[i]);
		}
	}
}
