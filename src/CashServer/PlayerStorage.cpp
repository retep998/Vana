...
void PlayerStorage::load() {
	...
	for (size_t i = 0; i < res.num_rows(); i++) {
		...
		item->setExpirationTime(mysqlpp::DateTime(row[3]));
...
void PlayerStorage::save() {
	...
	for (unordered_map<int64_t, CashItem *>::iterator iter = cashStorage.begin(); iter != cashStorage.end(); iter++) {
		...
		query << iter->first << ", "
			...
			<< mysqlpp::quote << mysqlpp::DateTime(time_t(iter->second->getExpirationTime())) << ")";
	}
	if (!firstrun)
		query.exec();
}
...
void PlayerStorage::checkExpiredItems() {
	...
	for (size_t i = 0; i < cashStorage.size(); i++) {
		item = cashStorage[i];
		if (item != nullptr && item->getExpirationTime() != -1 && item->getExpirationTime() <= time(0)) {
